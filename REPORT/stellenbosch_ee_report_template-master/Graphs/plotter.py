...
for dataset, divergence in divergences.items():
    plt.plot(keypoints, divergence, label=f"{dataset} (FLANN - BF)", marker='o')

# Labels and Title
plt.xlabel('Keypoints Target')
plt.ylabel('Divergence (FLANN - BFMatcher) in MAE GPS Error (m)')
plt.title('FLANN vs BFMatcher: Divergence At Different Keypoint Targets')
