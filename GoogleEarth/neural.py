...
extractor = SuperPoint(
    max_num_keypoints=50000,  # Maximum number of keypoints
    detection_threshold=0.00005,  # Detection threshold
...
matcher = LightGlue(  # Initialize LightGlue
    ...
    depth_confidence=-1, 
    width_confidence=-1,
    filter_threshold=0.1  # Custom filter threshold
).eval().to(device)
...
class UAVNavigator:
    ...
    def compute_pixel_shifts(self, feats1, feats2):
    ...
    def analyze_matches(self, lower_percentile):
        ...
        for i in reversed(range(1, len(self.stored_images))):  # iterate through all images in reverse order. i is the current image index
            ...
            for j in range(i):  # iterate through all images before the current image
                shifts = self.compute_pixel_shifts(self.stored_features[i], self.stored_features[j])  # compute pixel shifts between the current image and the previous image
                if shifts is not None and len(shifts) > max_good_matches:  # if the number of good matches is greater than the current max, update the max and the best index
                    ...
                    best_index = j
                    print(f"Number of good matches between images {i+1} and {j+1}: {max_good_matches}")

            if best_index != -1:
                best_index = i - 1  # TO DO: fix this. The method of most matches is not working. Must use correlation method. Namely, image 9 is matching with 7 not 8. 
                shifts = self.compute_pixel_shifts(self.stored_features[i], self.stored_features[best_index])
